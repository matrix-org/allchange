...
const FIX_MAGIC_WORDS = ["close", "closes", "closed", "fix", "fixes", "fixed", "resolve", "resolves", "resolved"];
...
test("Notes picked up from PR body (lowercase)", () => {
    const pr = mockPr();
    pr.body = ["Does a thing", "notes: this does a thing"].join("\n");
...
test("Headline picked up from PR body (lowercase)", () => {
    const pr = mockPr();
    pr.body = ["Does a thing", "headlines: this does a thing"].join("\n");
...
test("Project notes picked up from PR body", () => {
    const pr = mockPr();
    pr.body = ["Does a thing", "boopity-boo notes: this does a thing"].join("\n");
...
test("Fixes parsed from #123", () => {
    for (const magicWord of FIX_MAGIC_WORDS_ALLVARIANTS) {
        const pr = mockPr();
        // with colon
        pr.body = ["Does a thing", `this ${magicWord}: #123`].join("\n");
        ...
        // without
        pr.body = ["Does a thing", `${magicWord} #123`].join("\n");
...
test("Fixes parsed from owner/repo#123", () => {
    for (const magicWord of FIX_MAGIC_WORDS_ALLVARIANTS) {
        const pr = mockPr();
        // with colon
        pr.body = ["Does a thing", `this ${magicWord}: bert/llamalist#123`].join("\n");
        ...
        // without
        pr.body = ["Does a thing", `${magicWord} bert/llamalist#123#123`].join("\n");
...
test("Fixes parsed from issue URL", () => {
    for (const magicWord of FIX_MAGIC_WORDS_ALLVARIANTS) {
        const pr = mockPr();
        // with colon
        pr.body = ["Does a thing", `this ${magicWord}: https://github.com/bert/llamalist/issues/123`].join("\n");
        ...
        // without
        pr.body = ["Does a thing", `${magicWord} https://github.com/bert/llamalist/issues/123`].join("\n");
        ...
        // http
        pr.body = ["Does a thing", `${magicWord} http://github.com/bert/llamalist/issues/123`].join("\n");
