...
export async function githubOrgRepoFromDir(repoDir: string) {
    const pkgJson = JSON.parse(
        await fsProm.readFile(path.join(repoDir, "package.json"), {
            encoding: "utf8",
        }),
    );
...
export function getMergedPrs(repoDir: string, from: string, to: string): Promise<string[]> {
    ...
    return new Promise<string[]>((resolve) => {
        const proc = childProcess.spawn(
            "git",
            ["rev-list", "--merges", "--format=medium", "^" + hackRevision(from), hackRevision(to)],
            {
                cwd: repoDir,
            },
        );
...
export async function getPrInfo(repoOwner: string, repoName: string, prNumbers: string[]): Promise<PrInfo[]> {
